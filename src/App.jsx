...
const ADMIN_PIN = import.meta.env.VITE_ADMIN_PIN || '998877';


// 미리보기 및 로컬 테스트 호환용 설정
...
// 프로젝트 고유 ID (데이터베이스 경로용)
const APP_ID = typeof __app_id !== 'undefined' ? __app_id : 'edustock-2026';
// 관리자 PIN 번호
...
export default function App() {
  ...
  useEffect(() => {
    ...
    // 공용 시장 데이터
    const marketRef = doc(db, 'artifacts', APP_ID, 'public', 'state');
  ...
  const handleTrade = async () => {
    ...
    if (type === 'BUY') {
      if (newTokens < totalCost) { alert("토큰이 부족합니다."); return; }
      newTokens -= totalCost;
      ...
      if (!existing || existing.amount < amount) { alert("보유 주식이 부족합니다."); return; }
      newTokens += totalCost;
      const newAmount = existing.amount - amount;
  ...
  const executeAdminAction = async () => {
    ...
    if (adminAction.newsText.trim()) {
      ...
      newNews.unshift({
        id: Date.now(), time: timeString, text: adminAction.newsText.trim(),
